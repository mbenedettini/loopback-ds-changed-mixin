...
module.exports = function(Model, options) {
  ...
  Model.observe('after save', function(ctx, next) {
    ...
    if (ctx.hookState.changedItems && !_.isEmpty(ctx.hookState.changedItems.changedProperties)) {
      ...
      /* TODO: if more than one properties are assigned the same cb and both
        are changed in the same operation the cb will only run for the
        1st one with changeset passed for that 1st one, ignoring the
        changeset for the 2nd */
      ...
      async.forEachOf(properties, function(changeset, property, cb) {
        ...
        if (alreadyRun[callback]) {
          debug('skipping already run callback %s for', callback, property);
          return cb();
        }
        alreadyRun[callback] = true;
  ...
  Model.getChangedProperties = function(oldVals, newVals, properties) {
    ...
    _.forEach(properties, function(key) {
      ...
      if (newVals[key]) {
        ...
        if (!oldVals[key] || !_.isEqualWith(oldVals[key], newVal, function areEqual(a, b) {
          if (_.isDate(a) || _.isDate(b)) {
            return _.isEqual(new Date(a), new Date(b));
          }
          return a === b;
        })) {
